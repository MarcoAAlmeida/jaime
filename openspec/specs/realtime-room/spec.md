...
Provides the WebSocket connection to the room identified by the current
URL and relays pattern-code updates between the clients connected to it,
gated by track ownership (see the `track-ownership` capability). Room
creation/joining itself is covered by the `room-lifecycle` capability.
