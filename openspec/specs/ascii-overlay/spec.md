...
The system SHALL advance the panel to a newly, randomly selected piece
every Nth beat while the room's shared document is playing, where N is
a per-viewer setting each participant controls independently for their
own view — never synced to or shared with other participants in the
room. The system SHALL NOT advance the displayed piece while playback
is stopped.

#### Scenario: The piece advances during playback
- **WHEN** the room is playing and N beats have elapsed since the last
  advance, using the viewer's own current setting for N
...
- **WHEN** the panel displays a piece much smaller than the panel area
