...
- [x] 1.1 `app/lib/strudelEditor.ts` — add a `lineWrapping` `Compartment`
...
- [x] 1.2 `app/components/TrackEditor.vue` — drive `setLineWrapping` from
...
- [x] 1.3 `app/pages/app/composition/[id].vue` — same wiring off its
      editor host `ResizeObserver`.
- [x] 1.4 Verify: a line longer than the viewport wraps (content width ≤
...
- [x] 2.1 `app/pages/app/composition/[id].vue` — header row `flex-wrap`;
...
- [x] 3.1 `app/composables/useShareLink.ts` — `share()` = `navigator.share`
...
- [x] 3.2 Both room pages use it for the invite button (replaces the
      inline `copyInviteLink`); keep the `copy-invite-button` testid.
- [x] 3.3 Verify: with `navigator.share` present the share sheet is
      invoked with the room URL; without it, the link is copied and the
      UI confirms; the shared link opens the same room.

## 4. Tests + polish

- [x] 4.1 `e2e/` — a portrait phone-viewport spec (JAM room +
...
- [x] 4.2 Update the existing composition / multi-client e2e that click
      now-collapsed header controls — open the ⋯ menu first at narrow
      widths; desktop-width tests unchanged.
- [x] 4.3 `nuxt typecheck`, `npm test`, `playwright test` green.
