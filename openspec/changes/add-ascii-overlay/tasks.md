...
- [ ] 1.1 `app/pages/app/composition/[id].vue` — add a new `<aside>`,
...
- [ ] 1.2 Two hardcoded fixture pieces (one small, one large — e.g. a
...
- [ ] 1.3 Attribution element inside the panel: artist/title text
      linking to `sourceUrl`, pinned near the bottom of the panel.
- [ ] 1.4 Header toggle button beside `toggle-panel-button`
      (`data-testid="toggle-ascii-panel-button"`), flipping a
      `showAsciiPanel` ref independent of `panelOpen`; off by default
      at every breakpoint.
- [ ] 1.5 Mobile mutual exclusion: below `md`, opening the ASCII panel
...
- [ ] 1.6 Font-size scaling: compute the `<pre>`'s font size from the
...
- [ ] 1.7 A manual "next fixture" control (temporary, dev-only —
...
- [ ] 1.8 Playwright smoke check: toggling on/off shows/hides the
      ...
      holds below `md`.
- [ ] 1.9 Manual review with the user in the browser; adjust panel
      width/background/scaling clamp values as directed before
      starting slice 2.

## 2. Slice 2 — Scrape asciiart.eu into production data

- [ ] 2.1 `migrations/patterns/0007_ascii_gallery.sql` — create
...
- [ ] 2.2 `scripts/lib/ascii-gallery-scraper.mjs` — category-page
...
- [ ] 2.3 Parser safety: assert a minimum plausible piece count per
      subcategory page; fail loudly (non-zero exit, clear message)
      rather than silently writing an empty or truncated dataset.
- [ ] 2.4 `scripts/scrape-ascii-gallery.mjs` — CLI entry point: polite
      ...
      `db:migrate:*`.
- [ ] 2.5 `npm run` alias for the crawl (e.g. `ascii:scrape [--
      --remote]`) in `package.json`, documented as a manual operator
      command.
- [ ] 2.6 `scripts/lib/ascii-gallery-scraper.test.mjs` — unit tests for
...
- [ ] 2.7 Run `db:migrate:local` to apply `0007` locally; run the
      crawler against `--local` and sanity-check row counts/spot-check
      a few rows.
- [ ] 2.8 Apply migration `0007` to remote D1; run the crawler against
...
- [ ] 2.9 `server/catalog/asciiArt.ts` — `getRandomAsciiArt(db, {
      count })`, `getAsciiArtById(db, id)`, `isMigrated(db)` (mirrors
      `server/catalog/patterns.ts`).
- [ ] 2.10 `server/api/ascii-art/random.get.ts` —
      `GET /api/ascii-art/random?count=<n>`, clamping `count` to a
      sane max; returns a 503-style clean response if `isMigrated` is
      false.
- [ ] 2.11 `server/api/ascii-art/[id].get.ts` — direct lookup by id.
- [ ] 2.12 API unit/integration tests: random batch returns `count`
