...
- [ ] 1.1 `scripts/jah-eval/lib/replies.mjs`: move `fences`, `balanced`, the
      ...
      next line.
- [ ] 1.2 `scripts/jah-prompt-eval.mjs` imports the helpers instead of
      carrying copies; behaviour and printed output are unchanged
      (`node --check`, and the pinned helper tests from 1.1).
- [ ] 1.3 `package.json`: add `"jah:eval": "node scripts/jah-eval/run.mjs"`.
- [ ] 1.4 `scripts/jah-eval/README.md`: what the harness is, the case format
...
- [ ] 2.1 `lib/cases.mjs`: load `cases/*.mjs`, validate the whole set and
...
- [ ] 2.2 Message rendering: docs and compose send `message`; fix sends
...
- [ ] 3.1 `lib/functions.mjs`: `loadFunctionIndex()` returning
...
- [ ] 3.2 `lib/score.mjs`: `score(case, reply, evalOutcome)` → per-check
...
- [ ] 3.3 Evaluation adapter: evaluate the reply's primary block with
...
- [ ] 4.1 `lib/model.mjs`: `generateText` with the model id and
      ...
      model function.
- [ ] 4.2 Drift-guard test: reads `server/jah/reply.ts` as text and asserts
...
- [ ] 5.1 `lib/report.mjs`: prompt fingerprint (short sha-256 of the system
...
- [ ] 5.2 Baseline and comparison: `--save-baseline` writes
...
- [ ] 5.3 `run.mjs`: flags `--samples N` (default 3), `--kinds`, `--case <id>`,
...
- [ ] 5.4 `--validate`: structural checks, then evaluate every fix case's
...
- [ ] 6.1 Author about 20 `docs` cases: the 14 questions from
...
- [ ] 6.2 Author about 10 `fix` cases from realistic mistakes (an undefined
      ...
      rest.
- [ ] 6.3 Author about 10 `compose` cases with mechanically checkable
