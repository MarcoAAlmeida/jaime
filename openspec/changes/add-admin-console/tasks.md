...
- [ ] 1.1 `migrations/patterns/0006_ai_access_and_usage.sql` — `ALTER
...
- [ ] 1.2 `shared/user.ts` — `User` gains `aiAccess: boolean`.
...
- [ ] 2.2 `effectiveAccess(user, allowlist): 'flag' | 'allowlist' |
      'none'` for the roster's "why" column (flag wins the label when
      both are true).
- [ ] 2.3 Unit tests: flag-only, allowlist-only, both, neither,
      anonymous/undefined; allowlist parse (spaces, casing, trailing
      comma, empty string).

## 3. Operator gate

- [ ] 3.1 `server/utils/adminAuth.ts` — `isOperator(user)` (GitHub login
...
- [ ] 3.2 `app/middleware/operator.ts` — client route middleware:
...
- [ ] 3.3 Tests: `requireOperator` passes the operator, 404s a
      non-operator and an anonymous request.

## 4. Admin APIs

- [ ] 4.1 `GET /api/admin/users` — `requireOperator`, then
...
- [ ] 4.2 `PATCH /api/admin/users/[id]` — `requireOperator`, validate
      body `{ aiAccess: boolean }`, `UPDATE users SET ai_access = ?
      WHERE id = ?`, return the updated mapped row. 404 if the id is
      unknown.
- [ ] 4.3 `server/auth/aiUsage.ts` — `listRecentUsage(db, limit = 50)`
      → `ai_usage` rows newest first. (The write helper is
      `add-jah-chat`'s.)
- [ ] 4.4 `GET /api/admin/usage` — `requireOperator`, `limit` query
      (clamp 1–200, default 50), return `listRecentUsage`.
- [ ] 4.5 Pool-workers tests: each endpoint 404s a non-operator; the
      toggle flips `ai_access` and the roster reflects it; `usage`
      returns `[]` cleanly with an empty table.

## 5. Admin page

- [ ] 5.1 `app/pages/admin.vue` — `definePageMeta({ layout: 'landing',
      middleware: ['auth', 'operator'] })`. Two `UTable`s: accounts
...
- [ ] 5.2 Toggling a row calls `PATCH /api/admin/users/[id]` and
      updates the row in place; a failed write is surfaced, not
      swallowed.
- [ ] 5.3 No link to `/admin` from any nav — operator types the URL.
      (Confirm nothing renders it for non-operators.)

## 6. Tests + ship

- [ ] 6.1 e2e `e2e/admin.spec.ts` (reuses `OAUTH_E2E`): the operator
...
- [ ] 6.2 `nuxt typecheck`, `vitest run`, `playwright test` green.
- [ ] 6.3 `openspec validate add-admin-console --strict`.
