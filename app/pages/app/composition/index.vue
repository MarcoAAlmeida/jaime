...
function createRoom() {
  navigateTo(`/app/composition/${nanoid(10)}`)
}
...
function extractRoomId(input: string): string | null {
  ...
  try {
    ...
    const match = url.pathname.match(/\/composition\/([^/]+)/)
      ...
      <div class="mx-auto flex w-full max-w-sm flex-col gap-6 py-10">
        <div class="border-primary/25 bg-primary/5 rounded-lg border p-4">
          <h2 class="mb-1 font-medium">
            Start a Composition Room
          </h2>
          <p class="text-muted mb-3 text-sm">
            One shared script that everyone edits together — takes you
            straight in.
          </p>
